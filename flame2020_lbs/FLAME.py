...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        with open(config.flame_model_path, 'rb') as f:
            try:
                ...
                print('Caught numpy ImportError, trying to fix it. Consider upgrading numpy')
                from ..flame import numpy_fix
                numpy_fix()
                f.seek(0) # since we already tried to open the file
                ss = pickle.load(f, encoding='latin1')
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None, calc_landmarks=True, return_J_transformed=False):
        """
            Input:
